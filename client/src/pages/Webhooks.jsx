...
export default function Webhooks() {
  ...
  const register = async () => {
    ...
    setSubmitting(true);
    try {
      const r = await api.post("/webhook/register", { ...form, emailAlerts });
      setResult(r.data);
    ...
    <div className="max-w-3xl">
      <div className="mb-8">
        ...
        <div className="text-terminal-gray text-xs">
          Auto-scan on every GitHub push. Zero manual intervention.
        </div>
      </div>
      ...
      <div className="ascii-box p-5 mb-8" data-label="HOW IT WORKS">
        <div className="space-y-2 text-xs text-terminal-gray">
          <div>
            <span className="text-terminal-green">01.</span> Register your
            GitHub repo below → get a webhook URL + secret
          </div>
          <div>
            <span className="text-terminal-green">02.</span> Go to your repo
            Settings → Webhooks → Add webhook
          </div>
          <div>
            <span className="text-terminal-green">03.</span> Paste the URL, set
            content type to{" "}
            <span className="text-terminal-amber">application/json</span>, paste
            the secret
          </div>
          <div>
            <span className="text-terminal-green">04.</span> Select event:{" "}
            <span className="text-terminal-amber">Just the push event</span>
          </div>
          <div>
            <span className="text-terminal-green">05.</span> Every push
            auto-triggers a scan. Results appear in your dashboard.
          </div>
        </div>
      </div>
      ...
      <div className="ascii-box p-6 mb-8" data-label="REGISTER WEBHOOK">
        <div className="space-y-5">
          <div>
            <label className="text-terminal-green-dim text-xs block mb-2 uppercase tracking-widest">
              GitHub Repository
            </label>
            <div className="flex items-center gap-2">
              <span className="text-terminal-green text-xs flex-shrink-0">
                github.com/
              </span>
              <input
                type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, manifestFile: e.target.value }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, ecosystem: e.target.value }))
                }
                className="w-full px-3 py-2 text-xs"
                style={{
                  background: "#0a0a0a",
          ...
          <div className="flex items-center gap-3 mt-2">
            <button
              type="button"
              onClick={() => setEmailAlerts((e) => !e)}
              className={`w-10 h-5 border transition-all relative ${
                emailAlerts
                  ? "border-terminal-green bg-terminal-green-faint"
            ...
            >
              <span
                className={`absolute top-0.5 w-3 h-3 transition-all ${
                  emailAlerts
                    ? "right-0.5 bg-terminal-green"
                    : "left-0.5 bg-terminal-gray"
                }`}
              />
            </button>
            <span className="text-terminal-gray text-xs">
              Email alerts for CRITICAL/HIGH vulnerabilities
            </span>
          </div>
          {error && (
            <div className="term-border-red p-3 text-terminal-red text-xs">
              ERROR: {error}
            </div>
          )}
      ...
      {result && (
        <div className="term-border-green p-5 mb-8" style={{ borderRadius: 0 }}>
          <div className="text-terminal-green text-xs font-bold mb-3 glow-sm">
            ✓ WEBHOOK REGISTERED
          </div>
          <div className="space-y-3 text-xs">
            <div>
              <div className="text-terminal-gray mb-1 uppercase tracking-widest text-xs">
                Webhook URL
              </div>
            ...
            <div className="text-terminal-red text-xs">
              ⚠ Copy and store the secret now. It will not be shown again.
            </div>
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="divide-y divide-terminal-border">
            {webhooks.map((wh) => (
              <div
                key={wh._id}
                className="flex items-center justify-between p-4 hover:bg-terminal-green-faint"
              >
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => toggleAlerts(wh._id, !wh.emailAlerts)}
                      className={`w-10 h-5 border transition-all relative ${
                        wh.emailAlerts
                          ? "border-terminal-green bg-terminal-green-faint"
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 border ${wh.active ? "badge-low" : "badge-unknown"}`}
                  >
