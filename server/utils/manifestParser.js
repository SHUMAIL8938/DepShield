    ...
    switch (ecosystem) {
      ...
      case 'PyPI': {
        if (content.includes('[tool.poetry') || content.includes('[project]')) {
          ...
          for (const line of lines) {
            if (line.match(/dependencies\s*=/)) { inDeps = true; continue; }
            if (inDeps && line.startsWith('[')) { inDeps = false; }
            if (inDeps) {
              const match = line.match(/["']?([a-zA-Z0-9_-]+)["']?\s*[>=<!]/);
              if (match) deps.push({ name: match[1], version: extractVersion(line) });
            }
          }
        } else {
          const lines = content.split('\n');
          for (const line of lines) {
            ...
            if (match) deps.push({ name: match[1], version: match[2] || 'unknown' });
          }
        }
        break;
      }
      ...
      case 'Maven': {
        if (content.includes('<project')) {
          ...
          const matches = content.matchAll(/['"]([a-zA-Z0-9._-]+):([a-zA-Z0-9._-]+):([^'"]+)['"]/g);
      ...
      case 'RubyGems': {
        const matches = content.matchAll(/gem\s+['"]([^'"]+)['"]\s*(?:,\s*['"]([^'"]+)['"])?/g);
        for (const match of matches) {
          deps.push({ name: match[1], version: match[2]?.replace(/^[~>=<]/, '') || 'unknown' });
        }
        break;
      }
      ...
      case 'crates.io': {
        ...
        for (const line of lines) {
          ...
          if (inDeps) {
            ...
            if (simple) deps.push({ name: simple[1], version: simple[2].replace(/^[^0-9]*/, '') });
            else if (table) deps.push({ name: table[1], version: table[2].replace(/^[^0-9]*/, '') });
          }
        }
        break;
      }
